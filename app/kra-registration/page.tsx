    ...
    <PageBackground>
      <div className="container mx-auto px-4 py-8 max-w-5xl">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-3xl md:text-4xl font-heading mb-2 text-foreground">Register KRA PIN</h1>
          ...
          <CardHeader>
            ...
            <div className="mt-6">
              ...
              <div className="flex justify-between mt-2">
                {steps.map((s, index) => (
                  <div key={index} className={`flex flex-col items-center ${index < step ? 'text-brand-cyan' : 'text-muted-foreground'}`}>
                    <div className={`w-6 h-6 md:w-8 md:h-8 rounded-full flex items-center justify-center text-xs md:text-sm ${index < step ? 'bg-brand-cyan text-black shadow-lg shadow-brand-cyan/20' : 'bg-muted border border-border'}`}>
                      {index < step ? <Check className="w-4 h-4" /> : index + 1}
                    </div>
                    <span className="text-[10px] md:text-xs mt-1 text-center hidden md:block font-medium">{s}</span>
                  </div>
                ))}
              </div>
            </div>
              ...
              <div className="space-y-2">
                {progressSteps.map((step, index) => (
                  <div key={index} className="flex items-center gap-2">
                    ...
                    <span className={`text-sm ${step.done ? 'text-brand-green font-medium' : 'text-muted-foreground'}`}>
                      {step.text}
                    </span>
                  </div>
                ))}
              ...
              {currentProgressStep === "Waiting for OTP verification" && registrationStatus !== "error" && (
                <div className="space-y-4 pt-4 border-t">
                  ...
                  <Button
                    ...
                    className="w-full bg-brand-cyan hover:bg-brand-cyan/90 text-black shadow-lg shadow-brand-cyan/20 border-none"
                  >
          ...
          <DialogContent className="w-[90%] max-w-[500px] rounded-lg p-6">
            <DialogHeader>
              ...
              <div className="text-base space-y-3 pt-4">
                <Alert className="bg-brand-green/10 border-brand-green/20">
                  <CheckCircle className="h-4 w-4 text-brand-green" />
                  <AlertTitle className="text-brand-green font-heading">Your KRA PIN</AlertTitle>
                  <AlertDescription className="text-foreground">
                    <div className="text-2xl font-mono font-bold mt-2 text-brand-green">{newPIN}</div>
                  </AlertDescription>
                </Alert>
