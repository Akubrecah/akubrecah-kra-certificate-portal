...
import { Step1PIN, Step2Details, Step4Filing } from "./components/ReturnSteps"
  ...
  useEffect(() => {
    const initializeSession = async () => {
      try {
        ...
        // If ID is passed in URL, trigger search
        const idFromQuery = searchParams.get('id');
        if (idFromQuery) {
          console.log('[APP] Found ID in query params:', idFromQuery);
          // We need a way to communicate this to Step1PIN
          // Since Step1PIN is a child, we can't directly call its methods
          // But Step1PIN is controlled by FilePage.
          // Wait, Step1PIN has its own internal state for idNumber? 
          // Let's check Step1PIN props.
        }
      } catch (error) {
          ...
          <div className="space-x-4">
            {passwordValidationStatus !== "valid" && formData.activeTab === 'id' && (
              <Button
                ...
                className="bg-gradient-to-r from-brand-cyan to-blue-700 hover:from-brand-cyan hover:to-blue-800 text-black shadow-xl shadow-brand-cyan/20 transition-all active:scale-95"
              >
                Validate Password
              </Button>
            )}
  ...
  return (
    <PageBackground>
      <div className="flex min-h-screen flex-col items-center py-4 md:py-8 px-4">
        ...
        {isComplete ? (
          <Card className="border-green-200 bg-background/95 shadow-lg max-w-7xl w-full">
            ...
            <CardContent className="pt-12 space-y-6">
              <div className="grid grid-cols-2 gap-4 bg-secondary rounded-lg p-4 border border-border">
                <div>
            ...
            <CardFooter className="flex justify-center border-t p-4">
              <p className="text-xs text-muted-foreground">Need assistance? Contact our support at support@nungereturns.co.ke</p>
            </CardFooter>
          </Card>
        ) : (
            ...
            <DialogHeader>
              ...
              <DialogDescription asChild>
                <div className="text-foreground space-y-2 text-sm">
                  ...
                  <div>Progress: Step {existingSessionData?.current_step || 1} of 3</div>
...
const steps = ["Enter PIN", "Confirm Details", "File Returns"]