...
async function generateDailyReport(): Promise<void> {
  ...
  console.log(`[DAILY REPORT] Tasks: ${tasks.length} (completed: ${tasks.filter((t: any) => t.status === 'completed').length})`);
  console.log(`[DAILY REPORT] Tickets: ${tickets.length} (resolved: ${tickets.filter((t: any) => t.status === 'resolved').length})`);
  console.log(`[DAILY REPORT] Security events: ${events.length} (critical: ${events.filter((e: any) => e.severity === 'critical').length})`);
}
