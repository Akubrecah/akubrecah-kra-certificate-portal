...
export async function checkAllComponents(): Promise<ComponentCheck[]> {
  ...
  const downComponents = results.filter((r: any) => r.status === 'down');
  if (downComponents.length > 0) {
    console.error(`[AUTO-HEALER] ${downComponents.length} component(s) DOWN:`, downComponents.map((c: any) => c.component).join(', '));
  }
