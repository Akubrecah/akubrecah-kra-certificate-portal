    ...
    try {
      ...
      if (searchPin || searchId) {
        ...
        if (cached) {
          ...
          // it is guaranteed to be a fallback date generated at retrieval time. Invalidate it to trigger exact lookup.
          ...
          if (regDate && cached.createdAt) {
            ...
            if (regDate === cacheDay) {
              console.log(`[KRAService] Cached date "${regDate}" matches cache creation day "${cacheDay}". Treating as unverified fallback.`);
              regDate = '';
            }
          }
      ...
      // Prefer PIN Checker exact registration date. DWR sometimes returns migration date like 12/06/2018.
      ...
      if (!registeredDate) {
        ...
        console.log(`[KRAService] Empty registration date, fallback to current date: ${registeredDate}`);
      }
  ...
  async fetchEffectiveDateFromPinChecker(pin) {
    ...
    try {
      ...
      try {
        ...
        console.log(`[KRAService][PinChecker] Resolved workerPath via require.resolve: ${workerPath}`);
        console.log(`[KRAService][PinChecker] Resolved corePath via require.resolve: ${corePath}`);
      } catch (err) {
        console.warn(`[KRAService][PinChecker] require.resolve failed, falling back to static paths:`, err.message);
      ...
      for (const p of searchPaths) {
        ...
        if (fs.existsSync(testPath)) {
          ...
          console.log(`[KRAService][PinChecker] Local traineddata source verified at: ${testPath}`);
          break;
        }
      }
      ...
      if (foundTrainedData && sourcePath) {
        try {
          ...
          if (!fs.existsSync(destPath)) {
            console.log(`[KRAService][PinChecker] Copying eng.traineddata to /tmp for read-write access...`);
            fs.copyFileSync(sourcePath, destPath);
            console.log(`[KRAService][PinChecker] Copy complete.`);
          } else {
            console.log(`[KRAService][PinChecker] eng.traineddata already exists in /tmp.`);
          }
        } catch (copyErr) {
          console.error(`[KRAService][PinChecker] Failed to copy traineddata to /tmp:`, copyErr.message);
        }
      } else {
        console.warn(`[KRAService][PinChecker] eng.traineddata not found in search paths. Defaulting cachePath to: ${cachePath}`);
      }
      ...
      // Initialize Tesseract worker completely offline using local cachePath & disabled gzip compression
      ...
      // Parse the primary registration date using regex
      // KRA HTML changed/varies. Date usually follows "PIN Registration Date" or is simply the first valid date in a table cell.
