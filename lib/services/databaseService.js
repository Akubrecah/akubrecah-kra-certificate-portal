  ...
  async storePaymentData(paymentData) {
    try {
      ...
      await analyticsService.logActivity({
        ...
        description: `KES ${paymentData.amount} received from ${paymentData.phoneNumber || 'user'}`,
        metadata: { userId: paymentData.userId, amount: paymentData.amount, status: paymentData.status }
      });
  ...
  async storeTaxReturnData(returnData) {
    try {
      ...
      await analyticsService.logActivity({
        ...
        description: `${returnData.returnType} return filed for PIN ${returnData.pin}`,
        metadata: { userId: returnData.userId, pin: returnData.pin, status: returnData.status }
      });
  ...
  async createUser(userData) {
    try {
      ...
      await analyticsService.logActivity({
        ...
        description: `New user ${userData.full_name || userData.email} joined the platform`,
        metadata: { userId: data[0].id, email: userData.email }
      });
